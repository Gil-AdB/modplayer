...
impl Display {
    ...
    pub fn display(play_data: &PlayData, instruments: &Vec<Instrument>, view_port: ViewPort, display: &mut dyn FnMut(String)) {
        ...
        screen.add_line(format!("'{}' duration in frames: {:5} duration in ms: {:5} tick: {:3} pos: {:3X}/{:<3X}  row: {:3X}/{:<3X} bpm: {:3} speed: {:3} virt: {:2} filter: {:5}", play_data.name,
