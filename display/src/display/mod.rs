...
impl Display {
    pub fn render(
        ...
        // Show the file name on top of the embedded song title; many
        // modules either lack an internal name or share generic ones
        // ("untitled"), and the user needs the path to find a stuck file
        // in a long playlist. Capped at 24 chars to keep the header
        // single-line at typical 120-char widths.
        let file_trimmed = if play_data.file_name.is_empty() {
            String::new()
        } else {
            format!("[{}] ", Self::fixed_width(&play_data.file_name, 24))
        };
