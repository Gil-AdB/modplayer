...
use crate::producer_consumer_queue::{PCQHolder, ProducerConsumerQueue, AUDIO_BUF_SIZE};
...
impl SongState {
    ...
    pub fn start(&mut self, sample_rate: f32, display_cb: fn (&PlayData)) -> (Option<JoinHandle<()>>, Option<JoinHandle<()>>) {
        ...
        play_thread = Option::from(spawn(move || Self::callback(s1.get())));
        ...
        if self.display_cb.is_some() {
            display_thread = Option::from(spawn(move || {
                let s = s2.get();
                let mut tb_guard = s.triple_buffer_reader.clone();
    ...
    pub fn close(&mut self, handle: (Option<JoinHandle<()>>, Option<JoinHandle<()>>)) {
        ...
        self.q.get().quit();
        if handle.0.is_some() {
            handle.0.unwrap().join();
        }
        if handle.1.is_some() {
            handle.1.unwrap().join();
        }
    }
}
